...
class AppStore extends Component {
  ...
  tabClicked = tabId => {
    const filterItems = tabsList.filter(everyItem => everyItem.tabId === tabId)
    this.setState({activeTabId: filterItems[0].tabId})
  }
  ...
  render() {
    ...
    const filteredCategory = appsList.filter(
      eachItem => eachItem.category === activeTabId,
    )
    const filterSearchResults = filteredCategory.filter(eachItem =>
    ...
    return (
      <div className="app-store-container">
        ...
        <ul className="apps-container">
          {filterSearchResults.map(eachApp => (
